...
module.exports = {
  ...
  run: async (bot, message, args, ops) => {
    if (!args[0])
      return message.channel.send(`${config.error} **Please Enter A User!**`);
    let opponent =
    ...
    if (opponent.user.bot)
      return message.channel.send(
        `${config.error} **Cannot Play Against Bots!**`
      );
    if (opponent.user.id === message.author.id)
      return message.channel.send(
        `${config.error} **Cannot Play Against Yourself!**`
      );
    ...
    try {
      ...
      if (winner === "time")
        return message.channel.send(
          `${config.stopwatch} **Game Ended Due To Inactivity!**`
        );
      return message.channel.send(
        winner
          ? `${config.gw} **Congrats, ${winner}!**`
          : `${config.error} **Its A Draw!**`
      );
    } catch (err) {
      ops.games.delete(message.channel.id);
      throw err;
