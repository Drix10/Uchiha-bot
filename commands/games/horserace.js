...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        await message.channel.send(stripIndents`
			${config.dot} **Choose a horse to bet on:** _(Type the number)_
        ...
        if (!messages.size) return message.channel.send(`${config.error} Sorry, cant have a race with no bets!`);
        ...
        return message.channel.send(win ? `${config.gw} Nice job! Your horse won!` : `${config.error} Better luck next time!`, { files: [leaderboard] });
