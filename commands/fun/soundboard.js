    ...
    run: async (bot, message, args) => {
        ...
        if (!args[0]) {
            ...
            try {
                ...
                if (connection) {
                    ...
                    if (message.channel.permissionsFor(bot.user).has(['ADD_REACTIONS', 'READ_MESSAGE_HISTORY'])) {
                        ...
                        channel.leave();
                        return message.channel.send(`**${config.error} Missing Permission - [ADD_REACTIONS]!**`);
                    }
                } else {
                    return message.channel.send(`**${config.error} Could Not Join VC, Try Again!**`);
                }
            } catch {
                return message.channel.send(`**${config.error} Something Went Wrong Try Again!**`);
            }
            return null;
        } else {
            if (!sounds.includes(args[0])) return message.channel.send(`**${config.dot} Which Sound Do You Want To Play? Either ${list(sounds, 'or')}!**`);
            ...
            try {
                ...
                return message.channel.send(`${config.error} **Couldn't Join VC, Check My Permissions!**`);
            };
        };
    }
};