    ...
    user.presence.activities.forEach(activity => {
      if (activity.type === "CUSTOM_STATUS") {
        const embed = new MessageEmbed()
          ...
          .addField(
            `${config.arrow} **Status**`,
            `${config.dot} **Custom status** -\n${activity.emoji ||
              "No Emoji"} | ${activity.state}`
          )
          .setThumbnail(user.user.displayAvatarURL())
          .setFooter(message.guild.name, message.guild.iconURL())
          .setTimestamp();
        ...
        const sembed = new MessageEmbed()
          ...
          .setColor(config.themecolor)
          .setThumbnail(image)
          .addField(`${config.dot} **Type**`, "Playing")
          .addField(`${config.dot} **App**`, `${name1}`)
          .addField(`${config.dot} **Details**`, `${details1 || "No Details"}`)
          .addField(
            `${config.dot} **Working on**`,
            `${state1 || "No Details"}`
          );
        message.channel.send(sembed);
      } else if (
