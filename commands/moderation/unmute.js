...
module.exports = {
    ...
    run: async (bot, message, args) => {
        if (!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send("<a:pg:870080272698322984> **You Dont Have The Permissions To Unmute Someone!**");

        if (!message.guild.me.hasPermission("MANAGE_GUILD")) return message.channel.send("<a:pg:870080272698322984> **I Don't Have Permissions To Unmute Someone!**")
        ...
        if (!mutee) return message.channel.send("<a:pg:870080272698322984> **Please Enter A Valid User!**");
        ...
        if (!muterole) return message.channel.send("<a:pg:870080272698322984> **There Is No Mute Role To Remove!**")
        if (!mutee.roles.cache.has(muterole.id)) return message.channel.send("<a:pg:870080272698322984> **User is not Muted!**")
        try {
        mutee.roles.remove(muterole.id).then(() => {
            ...
            const sembed = new MessageEmbed()
                ...
                .setDescription(`<:yeah:868270172627173456> ${mutee.user.username} was successfully unmuted.`)
