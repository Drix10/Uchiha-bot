  ...
  run: async (bot, message, args) => {
    if (!message.guild.me.hasPermission("ATTACH_FILES"))
      return message.channel.send(
        `${config.error}**Missing Permissions - [ATTACH_FILES]!**`
      );
    if (!args[0])
      return message.channel.send(
        `${config.dot} **Which User Would You Like To Be The Base?**`
      );
    if (!args[1])
      return message.channel.send(
        `${config.dot} **Which User Would You Like To Put Over The Base?**`
      );
    let base =
    ...
    if (!base)
      return message.channel.send(`${config.error} **Base User Not Found!**`);
    let overlay =
    ...
    if (!overlay)
      return message.channel.send(
        `${config.error} **Overlay User Not Found!**`
      );
    ...
    try {
      ...
      return message.channel.send(
        `${config.error} Oh no, an error occurred: \`${err.message}\`. Try again later!`
      );
    }
  }
};
